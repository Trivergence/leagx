...
class PayoutScreen extends StatelessWidget {
            ...
            return !_payoutViewModel!.busy
                ? Stack(
                    children: [
                      Column(
                        children: [
                          ...
                          if (listOfExtAccounts.isNotEmpty &&
                              _payoutViewModel!.isPayoutAvailble)
                            SizedBox(
                                width: 200,
                                child: MainButton(
                      ...
                      if (!_payoutViewModel!.isPayoutAvailble)
                        const PayoutFooter(),
                    ],
                  )
                : const LoadingWidget();
  ...
  Future<void> _addBank() async {
    ...
    if (isConnected == true) {
      ...
      if (linkUrl != null) {
        Navigator.pushNamed(_context!, Routes.addPayoutDetails,
                arguments: linkUrl)
            .then((_) {
  ...
  void getData() {
    if (_payoutViewModel!.expressAccount != null) {
      listOfExtAccounts =
          _payoutViewModel!.expressAccount!.externalAccounts!.data;
      if (listOfExtAccounts.isNotEmpty) {
        last4 = listOfExtAccounts.first.last4;
  ...
  void _withdraw() async {
    if (userSummary!.coinEarned!.round() >= AppConstants.minimumWithdraw) {
      FormDialog.show(
          ...
          onPositiveBtnPressed: (enteredAmount, withdrawType) async {
            ...
            if (isConnected == true) {
              ...
              if (double.parse(withdrawAmount).round() <=
                  userSummary!.coinEarned!.round()) {
                Loader.showLoader();
                bool isTransfered =
                    await _payoutViewModel!.transferToUser(withdrawAmount);
                if (isTransfered == true) {
                  PayoutModel? payoutModel = await _payoutViewModel!
                      .payoutMoney(withdrawAmount, currency!,
                          listOfExtAccounts.first.id!);
                  if (payoutModel != null) {
                    ...
                    if (success == true) {
                      FancyDialog.showSuccess(
                          ...
                          onOkPressed: () async => await _payoutViewModel!
                              .updateCoins(_dashBoardViewModel),
                          description: loc.payoutDialogTxtSuccessDesc(
                              withdrawAmount + "\$"));
                    } else {
                      ToastMessage.show(
                          loc.somethingWentWrong, TOAST_TYPE.error);
                    }
                    Loader.hideLoader();
                  }
                } else {
      ...
      ToastMessage.show(loc.payoutTxtLessCoins + ". " + loc.errorWithdrawLimit,
          TOAST_TYPE.error);
    }
  }
}
